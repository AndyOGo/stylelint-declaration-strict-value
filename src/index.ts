...
const mapIgnoreValue = (ignoreValue: TOptionPrimitive) =>
  (isRegexString(`${ignoreValue}`) ? stringToRegex(`${ignoreValue}`) : new RegExp(`^${ignoreValue}$`))
 ...
 */
const ruleFunction = (properties: string | string[], options: ISecondaryOptions, context: LinterOptions): PostCSSPlugin =>
  (root: Root, result: Result) => {
    ...
    properties.forEach((property) => {
       ...
       */
      function lintDeclStrictValue(node: Declaration, longhandProp?: string, longhandValue?: string, isExpanded = false) {
        ...
        if (isExpanded && ignoreVariables && !validVar &&
          ignoreFunctions && !validFunc && cssValues(longhandProp, longhandValue) !== true) {
          return false
        }
...
const declarationStrictValuePlugin = stylelint.createPlugin(ruleName, ruleFunction as unknown as Plugin)
